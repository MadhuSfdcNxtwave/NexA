...
SQL_SYSTEM = (
    ...
    "Use the ON conditions from # Join hints, but ALWAYS normalize user_id on BOTH sides: "
    "REPLACE(a.user_id, '-', '') = REPLACE(b.user_id, '-', ''). "
    "Never write bare a.user_id = b.user_id (hyphen formats differ across every table). "
    "Never list two tables without a JOIN. "
...
ANALYSIS_SYSTEM = (
    ...
    "- For contextual feedback details (row-level), explain in plain English: "
    "WHEN it was submitted, WHAT it was about (trigger/page), WHAT was asked, "
    "and WHAT the user answered — do not dump opaque id columns as the story.\n"
